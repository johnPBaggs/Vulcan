 ...
 * AIDynamicWeightedSearch class
 * This class will implement a dynamic weighted A* search. It will implement an A* search
...
public class AIDynamicWeightedSearch : AISearchInterface
{
	
	AIDynamicWeightedSearchList openList; //this will be a list that contain all the non explored nodes in the navigation mesh
	AIPolygon[] polygonArray; //This is the polygon array that makes up the navigation mesh
	AIDynamicWeightedSearchNode  finalSolutionStart; //the last node in the solution
	...
	/*
	 * AIDynamicWeightedSearch is the constructor for this class. It will set all the variables to their initial values
	 ...
	 */
	public AIDynamicWeightedSearch(Vector3 goalToAdd, AIPolygon[] polygonArrayToAdd)
	{
	...
	*/
	...
	{
		openList = new AIDynamicWeightedSearchList (goalPosition, polygonArray.Length);
		...
		if (polygonArray [count].getHasAgent () == true) {
			AIDynamicWeightedSearch.startingH = (goalPosition - polygonArray[count].getCenterVector()).magnitude; 
