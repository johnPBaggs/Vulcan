...
public class AIDynBiDirOpList 
{
	
	AIDynBiDirOpNode[] heap; // references the front of the lists
	int[] indicesArray;
	...
	/* AIDynBiDirOpList method is a constructor for this class. It will set up the initial values for the search
	 * Parameter:	(Vector3) goalPositionToAdd
	 * 				(int) polygonArrayLength is the length of the polygonArray from the navigation mesh
	 * 				(AIDynBiDirOpAgent)searchToAdd is the search agent that will be using this list
	 */
	public AIDynBiDirOpList (Vector3 goalPositionToAdd, int polygonArrayLength, AIDynBiDirOpAgent searchToAdd)
	{
		numberOfNodesHeld = 0;
		goalPosition = goalPositionToAdd;
		inList = new bool[polygonArrayLength];
		indicesArray = new int[polygonArrayLength];
		heap = new AIDynBiDirOpNode[polygonArrayLength];
	...
	{
		AIDynBiDirOpNode tempNode = heap [first];
	 ...
	 * 				(AIDynBiDirOpNode) parentNodeToAdd is the parent of the node to be added
	 * 				(float) gCostToAdd is the getGFromStartingNode value to be stored in the node to be added
	 * Return: none
	 */
	public void addNode(AIPolygon polygonToAdd, AIDynBiDirOpNode parentNodeToAdd, float gCostToAdd)
	{
		AIDynBiDirOpNode newSearchNode;
		if(numberOfNodesHeld == 0)
			newSearchNode = new AIDynBiDirOpNode (polygonToAdd, parentNodeToAdd, gCostToAdd, goalPosition, AIDynBiDirOpSearch.startingH, 1);
		else
			newSearchNode = new AIDynBiDirOpNode (polygonToAdd, parentNodeToAdd, gCostToAdd, goalPosition, AIDynBiDirOpSearch.startingH, (parentNodeToAdd.getDoFN() + 1));
