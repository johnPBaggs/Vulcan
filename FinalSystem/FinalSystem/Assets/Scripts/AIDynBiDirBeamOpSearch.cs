...
public class AIDynBiDirBeamOpSearch  : AISearchInterface
{

	
	AIDynBiDirBeamOpAgent agentToGoal; //the search going from the movement agent to the goal
	AIDynBiDirBeamOpAgent goalToAgent; //the search going from the goal to the agent
	...
	public void startSearch()
	{
		...
		while (pathFound == false) { //having found a path yet
			if(polygonArray.Length >= 4) //checks to see if the polygonArray is long enough for splitting the search into two
			{
				returnValueFromSearch = agentToGoal.doSearch(polygonArray.Length/4, goalToAgent.getCloseList()); //tells the first search to search for a small amount of time
				Debug.Log(Time.realtimeSinceStartup + " return Value = " + returnValueFromSearch);

				if(returnValueFromSearch == 1) //the goal was found
				{
					pathFound = true;
					Debug.Log(Time.realtimeSinceStartup + "agentToGoal");
				...
				else if(returnValueFromSearch == -1)
					return;
				else
				{
					returnValueFromSearch = goalToAgent.doSearch(polygonArray.Length/4, agentToGoal.getCloseList()); //tells the second search to search for a small amount of time
					Debug.Log(Time.realtimeSinceStartup + " return Value = " + returnValueFromSearch);

					if(returnValueFromSearch == 1)//agent was found
					{
						pathFound = true;
						Debug.Log(Time.realtimeSinceStartup + "goaltoAgent");
	...
	void getPathFromBiDirectionAgent(AIDynBiDirBeamOpAgent agentThatFoundPath)
	{
		...
		for (int count = 1; count < finalSolutionArray.Length; count++) {
			...
			//Debug.Log (Time.realtimeSinceStartup + "  id = " + finalSolutionArray [count].getID());
		}
